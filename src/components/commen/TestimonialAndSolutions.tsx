      ...
      <div className="modern-container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {reviews.slice(0, visibleReviews).map((testimonial, index) => (
            <div
              key={index}
              className="modern-card p-6 h-80 flex flex-col text-dark-700 hover:shadow-xl transition-all duration-300"
            >
              {/* Quote Icon */}
              ...
              <div className="flex-1 mb-6">
                <p className='text-sm leading-relaxed text-dark-600'>{testimonial.review}</p>
              </div>
              
              {/* Name and Stars Row - Fixed at bottom */}
              <div className="mt-auto">
                <div className="flex flex-col items-start space-y-2">
                  <span className="font-bold text-lg text-dark-900">{testimonial.name}</span>
                  <div className="flex space-x-1">
                    {[...Array(testimonial.stars)].map((_, i) => (
                      <FaStar key={i} className="w-4 h-4 text-primary-500" />
                    ))}
                  </div>
                </div>
              </div>
